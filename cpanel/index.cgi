...
sub handle_api_request_direct {
    ...
    if ($action eq 'health') {
        ...
        my $plugins = detect_2fa_plugins($cpanel_user, $payload->{site_path});
        print_json_success($plugins);
    }
    elsif ($action eq 'get_status') {
        print_json_success({ version => $PLUGIN_VERSION });
    }
    else {
...
sub scan_wordpress {
    ...
    foreach my $site (@sites) {
        # Use Cwd::realpath to resolve symlinks
        use Cwd 'realpath';
...
sub create_temp_user {
    ...
    write_audit_log($cpanel_user, 'META_ADD', "user=$username cmd=$meta1_cmd", "exit=$meta1_exit output=$meta1_output");
    write_audit_log($cpanel_user, 'META_ADD', "user=$username cmd=$meta2_cmd", "exit=$meta2_exit output=$meta2_output");

    # Verify metadata was set (defensive check)
    if ($meta1_exit != 0 || $meta2_exit != 0) {
        write_audit_log($cpanel_user, 'META_ADD_WARNING', "user=$username", "One or more metadata additions may have failed");
    }
    ...
    if ($disable_2fa_plugin ne '') {
        if ($disable_2fa_plugin =~ /^[a-zA-Z0-9_-]+$/) {
            write_audit_log($cpanel_user, '2FA_DISABLE_ATTEMPT', "user=$username site=$site_path plugin=$disable_2fa_plugin", "attempting");

            my $deactivate_cmd = sprintf('%s plugin deactivate %s --path="%s" 2>&1',
                $wp, quotemeta($disable_2fa_plugin), $site_path);
            my ($deactivate_output, $deactivate_exit) = run_wp_cli($deactivate_cmd);

            if ($deactivate_exit == 0) {
                $disabled_plugin_slug = $disable_2fa_plugin;
                write_audit_log($cpanel_user, '2FA_DISABLE_SUCCESS', "user=$username site=$site_path plugin=$disable_2fa_plugin", "deactivated");
            } else {
                write_audit_log($cpanel_user, '2FA_DISABLE_FAILED', "user=$username site=$site_path plugin=$disable_2fa_plugin", "error: $deactivate_output");
            }
        } else {
            write_audit_log($cpanel_user, '2FA_DISABLE_INVALID', "user=$username plugin=$disable_2fa_plugin", "invalid slug format");
        }
    }
    ...
    write_audit_log($cpanel_user, 'CREATE_USER_SUCCESS', "user=$username site=$site_path days=$days 2fa_disabled=$disabled_plugin_slug", "success");
...
sub list_temp_users {
    ...
    if ($exit_code == 0) {
        ...
        foreach my $user (@$all_users) {
            ...
            if ($is_temp eq '1') {
                my $expires_cmd = qq{$wp user meta get "$username" wp_temp_expires --path="$site_path" 2>&1};
...
sub delete_temp_user {
    ...
    my ($id_output, $id_exit) = run_wp_cli($id_cmd);
    chomp $id_output if $id_output;
    my $user_id = ($id_exit == 0 && $id_output =~ /^(\d+)$/) ? $1 : undef;

    write_audit_log($cpanel_user, 'DELETE_USER_ATTEMPT', "user=$username id=$user_id site=$site_path", "lookup_exit=$id_exit");

    if ($id_exit != 0) {
        # User doesn't exist in WordPress - re-activate 2FA if needed, then clean registry
        write_audit_log($cpanel_user, 'DELETE_USER_NOT_FOUND', "user=$username site=$site_path", "User not found in WordPress, cleaning registry");
    ...
    write_audit_log($cpanel_user, 'DELETE_USER_CMD', "user=$username site=$site_path cmd=$cmd", "exit_code=$exit_code output=$output");

    # Verify user was actually deleted
    my $verify_cmd = qq{$wp user get "$username" --field=ID --path="$site_path" 2>&1};
    my ($verify_out, $verify_exit) = run_wp_cli($verify_cmd);

    if ($verify_exit == 0) {
        # User still exists! Try fallback: delete by user ID
        write_audit_log($cpanel_user, 'DELETE_USER_STILL_EXISTS', "user=$username id=$user_id site=$site_path", "Trying delete by ID");

        if ($user_id) {
            my $id_delete_cmd = qq{$wp user delete $user_id --yes --path="$site_path" 2>&1};
            my ($id_del_out, $id_del_exit) = run_wp_cli($id_delete_cmd);

            write_audit_log($cpanel_user, 'DELETE_USER_BY_ID', "id=$user_id site=$site_path", "exit=$id_del_exit output=$id_del_out");

            # Verify again
            my ($final_out, $final_exit) = run_wp_cli($verify_cmd);
            if ($final_exit == 0) {
                write_audit_log($cpanel_user, 'DELETE_USER_FAILED', "user=$username site=$site_path", "User persists after both delete attempts");
                print_json_error('delete_failed', "Failed to delete user from WordPress. WP-CLI output: $output / ID delete: $id_del_out");
                return;
            }
        } else {
            write_audit_log($cpanel_user, 'DELETE_USER_FAILED', "user=$username site=$site_path", "No user ID for fallback");
    ...
    write_audit_log($cpanel_user, 'DELETE_USER_SUCCESS', "user=$username site=$site_path", "Verified deleted from WordPress");
...
sub detect_2fa_plugins {
    ...
    unless (validate_site_path($site_path, $cpanel_user)) {
        write_audit_log($cpanel_user, 'DETECT_2FA_INVALID_PATH', "site=$site_path", 'failed');
        return [];
    }
    ...
    write_audit_log($cpanel_user, 'DETECT_2FA', "site=$site_path", "found=" . scalar(@detected));
    return \@detected;
}
...
sub reactivate_2fa_if_safe {
    ...
    foreach my $entry (@{$registry->{users}}) {
        ...
        if ($entry->{site_path} eq $site_path &&
            ...
            write_audit_log($cpanel_user, '2FA_REACTIVATE_SKIP', "user=$username site=$site_path plugin=$plugin_slug",
    ...
    if ($exit_code == 0) {
        write_audit_log($cpanel_user, '2FA_REACTIVATE_SUCCESS', "user=$username site=$site_path plugin=$plugin_slug", "re-activated");
    } else {
        write_audit_log($cpanel_user, '2FA_REACTIVATE_FAILED', "user=$username site=$site_path plugin=$plugin_slug", "error: $output");
    }
}
...
###############################################################################
# WP-CLI Path Detection and Execution
###############################################################################
